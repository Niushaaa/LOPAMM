...
(1) per-game parlay P&L vs M for LOPAMM vs ind; (2) LOPAMM-vs-ind per-game scatter;
(3) per-game LOPAMM/ind loss ratio vs M. (native is reported in the table, not plotted)."""
import json
...
COL = {"ind": "tab:red", "LOPAMM": "tab:blue"}
...
def main():
    ...
    ap = np.array([r["lopamm_parlay"] for r in d])
    ...
    ax.scatter(M, np.clip(ap, lo, hi), s=34, color=COL["LOPAMM"], marker="o", alpha=0.75, label="LOPAMM")
    ...
    # (2) LOPAMM vs ind per game
    fig, ax = plt.subplots(figsize=(6.0, 6.0))
    lim = float(max(np.abs(ind).max(), np.abs(ap).max())) * 1.05
    ax.plot([-lim, lim], [-lim, lim], "--", color="0.5", lw=1.2, label="LOPAMM = ind")
    ax.axhline(0, color="0.8", lw=0.8, zorder=0)
    ax.axvline(0, color="0.8", lw=0.8, zorder=0)
    below = int((ap <= ind + 1e-9).sum())
    ax.scatter(ind, ap, s=34, color=COL["LOPAMM"], alpha=0.75, zorder=3)
    ax.set_xlim(-lim, lim); ax.set_ylim(-lim, lim)
    ax.set_xlabel("ind parlay loss (per game)")
    ax.set_ylabel("LOPAMM parlay loss (per game)")
    ax.legend(frameon=False, loc="upper left")
    ax.text(0.97, 0.04, f"LOPAMM $\\leq$ ind in {below}/{len(d)} games\n"
            f"corpus: ind {ind.sum():.0f}  vs  LOPAMM {ap.sum():.0f}",
            transform=ax.transAxes, ha="right", va="bottom", fontsize=9)
    ax.set_aspect("equal")
    ax.grid(alpha=0.3)
    fig.tight_layout()
    for ext in ("png", "pdf"):
        fig.savefig(f"{OUT}/fig_lopamm_vs_ind.{ext}", dpi=200)
    plt.close(fig)

    # (3) loss gap (ind - LOPAMM) vs M -- gap > 0  <=>  LOPAMM loses less (sign-robust)
    gap = ind - ap
    ...
    ax.set_ylim(lo, hi * 1.05)
    ax.set_xlabel("number of base markets $M$")
    ax.set_ylabel("loss gap   ind $-$ LOPAMM   (>0 ⇒ LOPAMM better)")
    ...
    ap_l = [lvlsum("lopamm_by_level", j) for j in levels]
    x = np.arange(len(levels)); w = 0.4
    fig, ax = plt.subplots(figsize=(7.2, 5.0))
    ax.bar(x - w / 2, ind_l, w, color=COL["ind"], label="ind")
    ax.bar(x + w / 2, ap_l, w, color=COL["LOPAMM"], label="LOPAMM")
    ax.axhline(0, color="0.6", lw=1, zorder=0)
    ...
    print(f"saved fig_parlay_loss_vs_M, fig_lopamm_vs_ind, fig_loss_gap_vs_M, "
    ...
    for j, il, al in zip(levels, ind_l, ap_l):
        print(f"  |S|={j}:  ind={il:11.1f}  lopamm={al:9.2f}")
