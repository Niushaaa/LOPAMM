...
Tests whether realized operator net loss scales as the theory predicts:
    LOPMM  stays pinned near the base floor,
...
CHECK_ROUTE = False        # opt-in eq.route guard for LOPMM sub-trades (--check_route)
...
def _assert_route(design, struct, Sp, tau_Sp, b):
    """Guard (LOPMM only): right after a sub-trade on Sp, residual[Sp] must equal
...
def run(cfg):
    ...
    print("EXPERIMENT 1  (low-order informed flow)   LOPMM / ind / base")
    ...
    def mline(tag, M, kM, supp, ntr):
        print(f"  M={M} k={kM} |D|={supp:3d} trades={ntr:6d} {tag} "
              ...
              f"LOPMM={summary[(M,'LOPMM')]['net_loss'][0]:9.3f}  "
...
def write_outputs(cfg, summary):
    ...
    # H1-H4 verdicts
    emit()
    gap = [summary[(M, "ind")]["net_loss"][0] - summary[(M, "LOPMM")]["net_loss"][0]
           for M in M_list]
    floor_ok = all(summary[(M, "base")]["net_loss"][0]
                   <= summary[(M, "LOPMM")]["net_loss"][0] + 1e-6
                   <= summary[(M, "ind")]["net_loss"][0] + 1e-6 for M in M_list)
    kl_ok = all(summary[(M, "LOPMM")]["kl"][0]
                <= 1.10 * summary[(M, "ind")]["kl"][0] for M in M_list)
    emit(f"  H1 LOPMM near floor, gap(ind-LOPMM) widens: {[round(g,2) for g in gap]}"
         f" -> {'YES' if gap[-1] > gap[0] else 'NO'}")
    emit(f"  H3 KL(LOPMM) <~ KL(ind): {'YES' if kl_ok else 'NO'}")
    emit(f"  H4 base <= LOPMM <= ind pointwise: {'YES' if floor_ok else 'NO'}")
...
def make_plots(cfg, summary):
    ...
    M_list = cfg["M_list"]
    colors = {"LOPMM": "tab:blue", "ind": "tab:red", "base": "tab:green"}
...
def parse_args():
    ...
    p.add_argument("--check_route", action="store_true",
                   help="assert eq.route holds after each LOPMM sub-trade")
