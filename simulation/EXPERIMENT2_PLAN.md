...
magnitude of belief updates) and showing LOPMM's net loss stays low/bounded while ind
...
- Ranges kept near/inside the low-order regime (`k=O(1)`, bounded magnitude): `B_θ≤5`,
  `R≤10`, `k≤8` — beyond these LOPMM's buys-only leak dominates.
- The LOPMM curve is drawn as **mean with a min→max range whisker** at each point (not a
  shaded band).
- **R = T/M** is the re-quote frequency: at low R (few trades) LOPMM's buys-only leak is
...
(LOPMM / ind / base means) plus LOPMM's **exact per-seed min–max band** — the Exp 1 figure
...
- **LOPMM robust to order / sparsity / magnitude:** stays low/flat (near or below the base
  floor) across the **ρ, B_θ, k** sweeps — the headline robustness claim.
- **R (frequency) is the boundary:** LOPMM's net loss **grows with R** (more re-quotes ⇒
...
- **gap `ind−LOPMM`** stays wide across ρ, B_θ, k; it narrows only at large R (the leak).
...
1. For each of the 4 sweeps, for each value: run the Exp 1 engine at M=10 with that knob
   ...
   cached result.
2. Read per-seed net loss (LOPMM/ind/base) from the `result_cache` bundle for each point.
3. Plot net loss vs the swept parameter (means + LOPMM min–max band), one panel per sweep.
...
1. **Re-quote-frequency ablation — now the R sweep (§3), good.** Frame R=T/M as the
   mechanism axis in the paper: LOPMM bounded at low R, leaking as R grows, ind
...
2. **Price-accuracy panel (H3).** Show LOPMM isn't buying its low loss with worse prices:
   KL(quoted ‖ true marginal) vs M (or vs the swept param). If `KL(LOPMM) ≲ KL(ind)`, it
   rules out "LOPMM just mis-quotes." One small figure.
3. **Informed-trader-profit histogram.** The trader-profit ↔ operator-loss identity is a
   ...
   much tighter distribution against LOPMM. A histogram figure makes "LOPMM is robust to
...
5. **Parameter-count / subsidy comparison.** LOPMM stores `3^M−1` shared parameters (one
