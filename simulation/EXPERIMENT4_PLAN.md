...
actual Kalshi NBA same-game-parlay order flow through LOPMM, ind, and base, settle at the
**realized** game outcomes, and compare operator net loss. Thesis (as in Exp 1): on real
flow, **LOPMM's parlay loss ≪ ind's**, with base as the floor (no parlays).
...
- **LOPMM** = `DesignA`: `Q(Sp)=Σ_{T⊆Sp} residual[T]` — pricing a sub-market reuses the lower
...
- **base** = only **genuine base-market trades** (real base flow). It does **not** take the
  ...
  is the base-markets-only floor; its singletons therefore diverge slightly from LOPMM/ind's
...
- **total loss** per design (`base_total`, `ind_total`, `lopmm_total`);
- **parlay loss** `= total − base_total` — the incremental cost of offering parlays over
  the genuine-base-only floor (the LOPMM-vs-ind signal; base parlay loss ≡ 0).

---

## 5. Metrics & figures

Per game: `M`, `maxK`, parlay-trade count, per-level loss, and (base / ind / LOPMM / native)
parlay and total loss. Figures (LOPMM vs ind; **native is table-only**):
- **loss by parlay order** (`fig_loss_by_order`, headline) — grouped bars of corpus total
  loss at each `|S|=2..8`, ind vs LOPMM: ind humps at 4–5 legs (~52k), LOPMM ≈ 0 everywhere;
- **parlay loss vs M** (`fig_parlay_loss_vs_M`, y clipped, off-scale ind noted);
- **loss gap `ind − LOPMM` vs M** (`fig_loss_gap_vs_M`) — gap>0 ⇒ LOPMM loses less
  (sign-robust): gap>0 in 64/83 games, corpus gap +174,081;
- (`fig_lopmm_vs_ind` scatter kept but de-emphasized — the huge dynamic range makes it hard
  to read).

Output: `results_exp4_kalshi/per_game.json`, `fig_parlay_loss_vs_M.png`,
`fig_lopmm_vs_ind.png`, `fig_loss_gap_vs_M.png`, `fig_loss_by_order.png`
...
conjunction under ind (own LMSR) and LOPMM (marginalize touched residuals, §2), record cash
...
(`exp4_validate.py`, 8 seeds): per-design total loss (base, ind, LOPMM) matches the dense
...
LOPMM prices the whole parlay market at **essentially zero incremental cost** (parlay P&L
...
| LOPMM | −25 | −27 | 3 | 19 | 2 | −2 | 0 |

ind humps at 4–5 legs (per-parlay cost rises with order, but high-order parlay *volume*
falls, so the product peaks mid-order); **LOPMM ≈ 0 at every order** (|·|≤27). Level 1
(singletons) is identical for ind/LOPMM and equals the base floor.
...
| LOPMM | −43 | **18,712** ≈ floor |
| native (order book) | — | (size-weighted, reported separately) |

**LOPMM's total ≈ the base floor** (18,712 vs 18,755): offering the full parlay book costs
LOPMM almost nothing, whereas `ind`'s total (192,793) is ~10× the floor. The **native**
